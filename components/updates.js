...
export default function Component() {
  ...
  const submitEmail = async e => {
    ...
    if (isEmailValid(email)) {
      ...
      window.alert(
        `Thanks for signing up! Your email ${email} is on the list.`
      );
    } else {
  ...
  return (
    <div id="signup" className="bg-gray-50 py-16 sm:py-24">
            ...
            <div className="relative">
              <div className="sm:text-center">
                ...
                <p className="mt-6 mx-auto max-w-2xl text-lg text-green-200">
                  I send out a weekly progress report on my path to 10K MRR. Why
                  not learn a bunch of things <b>NOT</b> to do from someone
                  else?
                </p>
              </div>
              {isRegistered && (
                <p className="mt-8 text-2xl text-center font-extrabold text-white tracking-tight sm:text-4xl">
                  You're on the list!
                </p>
              )}
              {!isRegistered && (
